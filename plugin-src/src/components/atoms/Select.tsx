  ...
  (
    ...
    return (
      <div className={cn("space-y-1.5", fullWidth && "w-full", className)}>
        {label && (
          <label className="text-neutral-700foreground ml-1 text-xs font-semibold tracking-wider uppercase">
            {label}
          </label>
        )}
        ...
        <div className="relative">
          {leftIcon && (
            <div className="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-neutral-700">
              {leftIcon}
            </div>
          )}
          ...
          <div className="pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 text-neutral-700">
            <ChevronDown size={16} />
          </div>
        </div>

        {helperText && !error && (
          <p className="text-neutral-700foreground ml-1 text-xs font-medium">
            {helperText}
          </p>
        )}

        {error && (
          <div className="animate-in slide-in-from-left-1 fade-in ml-1 flex items-center gap-1.5 text-red-500 duration-200">
            <AlertCircle size={12} />
