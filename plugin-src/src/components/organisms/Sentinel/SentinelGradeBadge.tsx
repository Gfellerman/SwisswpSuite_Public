...
const SIZE_CLASSES: Record<
  NonNullable<SentinelGradeBadgeProps["size"]>,
  string
> = {
  sm: "w-7 h-7 text-xs",
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <div
        ...
        className={`flex items-center justify-center rounded-2xl border-2 font-black select-none ${SIZE_CLASSES[size]} ${GRADE_STYLES[grade]}`}
      >
      ...
      {showLabel && (
        ...
        <span
          aria-hidden="true"
          className={`text-xs font-black tracking-widest uppercase ${GRADE_LABEL_COLOR[grade]}`}
        >
          {GRADE_LABELS[grade]}
        </span>
      )}
